use crate::lua::VM;
use crate::userdata::{UserData, UserDataMethods, UserDataFields};
...
impl UserData for Counter {
    ...
    fn add_methods<'v, M: UserDataMethods<'v, Self>>(methods: &mut M) {
        ...
        methods.add_meta_method("__tostring", |_vm, counter, _value| {
            Ok(Value::String(Box::new(format!("Counter({})", counter.get_count()))))
        });
        ...
        methods.add_meta_method("__add", |_vm, counter, value| {
            if let Value::Integer(n) = value {
                ...
                Err(SiltError::ExpOpValueWithValue(
...
// Example of using the UserData API
pub fn example_userdata<'gc>(vm: &mut VM<'gc>, mc: &Mutation<'gc>) {
    ...
    // Create a UserData value and set it as a global
    let counter_value = vm.create_userdata(mc, counter);
    vm.globals.borrow_mut(mc).insert("counter".to_string(), counter_value);
